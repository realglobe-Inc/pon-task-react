...
function define (srcDir, destDir, options = {}) {
  ...
  const shouldCompile = async (src, dest) => {
    const srcMTime = await mtimeOf(src)
    if (!srcMTime) {
      return true
    }
    ...
    for (const ext of ['.pcss', '.scss', '.less', '.css']) {
      const cssMTime = await mtimeOf(src.replace(/.jsx$/, ext).replace(/.js$/, ext))
      const cssChanged = cssMTime && (destMTime < cssMTime)
      if (cssChanged) {
        return true
      }
    }
  ...
  const compiler = async (code, inputSourceMap = null, options = {}) => {
    ...
    const skip = !watching && !(await shouldCompile(src, dest))
    if (skip) {
      return [null, null]
    }
